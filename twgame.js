...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookie) {
            ...
            await getid("showSecondFloorGameInfo", "game")
            ...
            try {
                if (err) {
                    ...
                    if (data.data && data.code === "0" && data.data.result) {
                        ...
                        if (result.activityGameInfo) {
                            $.awardName = result.activityGameInfo.gameCurrentRewardInfo.awardName
                            $.encryptAssignmentId = result.activityGameInfo.gameCurrentRewardInfo.encryptAssignmentId
                            $.secCode = result.activityGameInfo.gameCurrentRewardInfo.secCode
...
function togame(source) {
    return new Promise(async (resolve) => {
            body =  `{"source":"${source}","activityId":${$.actid},"encryptProjectId":"${$.encryptProjectId}","encryptAssignmentId":"${$.encryptAssignmentId}","secCode":${$.secCode}}`   
...
function taskPostUrl(functionid, body) {
    const time = Date.now();
    return {
        url: `https://api.m.jd.com/api?functionId=${functionid}&uuid=2603633656669366-8356360313267343&appid=ProductZ4Brand&client=wh5&t=${time}&body=${encodeURIComponent(body)}`,
        body: "",
        headers: {
            ...
            Cookie: cookie,
            Host: "api.m.jd.com",
            Referer: "https://h5.m.jd.com/babelDiy/Zeus/gtgo6k1L5DDSDmw6MihtNRskgK8/index.html?app_home_xview=1&lng=114.122007&lat=22.540418&sid=3188ee6bae21575b16b3950aebfb799w&un_area=19_1607_47388_62139",
